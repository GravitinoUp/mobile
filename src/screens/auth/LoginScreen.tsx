  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={styles.loginScreenScrollView}>
        ...
        <Text style={styles.description}>{AppStrings.signInDescription}</Text>
        <FormControl isInvalid={error !== null && error !== ''} style={{ flex: 1, flexDirection: "column", paddingBottom: 20 }}>
          <AppTextInput
          ...
          <AppTextInput
            ...
            onTrailingIconPress={() => onChangePasswordHidden(!isPasswordHidden)}
          />
          <FormControl.ErrorMessage leftIcon={<WarningOutlineIcon />}>
            {error}
          </FormControl.ErrorMessage>
        </FormControl>
        <View style={{ flex: 1, flexDirection: 'column', gap: 12, paddingBottom: 20 }}>
          <AppCheckbox label={AppStrings.rememberSignIn} isChecked={isChecked} onChange={(checked) => setChecked(checked)} />
        ...
        <View style={{ flex: 1, flexDirection: 'column', gap: 8 }}>
          <AppButton
            onPress={handleLogin}
            text={"Войти"} />
          {/* <TouchableOpacity style={{ marginTop: 12 }} onPress={() => navigation.navigate("RegisterScreen")}>
            <Text style={styles.registerUpper}>{AppStrings.noAccount}</Text>
            <Text style={styles.registerLower}>{AppStrings.register}</Text>
          </TouchableOpacity> */}
        </View>
        <View style={{ flex: 3 }} />
      </ScrollView>
