...
import { ChevronLeftIcon, IconButton, Row, ScrollView, useToast } from "native-base";
...
export default function CloseOrderScreen({ navigation, route }: any) {
  ...
  useEffect(() => {
    if (route.params.files) {
      ...
      AsyncStorage.setItem(`order-${order.order_id}`, JSON.stringify(data));
    }
  }, [route.params.files]);
  ...
  const deleteEmptyFiles = async (data: any) => {
    ...
    await AsyncStorage.setItem(`order-${order.order_id}`, JSON.stringify(data));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: AppColors.background }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollView}>
        <AppCard style={styles.cardContent}>
          ...
          />
          <Text style={{ marginTop: 13, color: AppColors.bodyLight }}>{AppStrings.attachFilesDescription}</Text>
          {!isLoading ? <AttachmentsCard
            style={{ marginTop: 13 }}
            attachments={files}
            canDelete={true}
            canAddMore={true}
            canAddFiles={true} // TODO Role check
            onAddFilePress={async () => { await pickImages() }}
            onFileDeletePress={async (index) => { await deleteFile(index) }}
            onMakePhotoPress={async () => {
              const result = await requestPermission();

              if (result) {
                navigation.navigate("CameraScreen", { files: files });
              }
            }}
      ...
      <BottomBar style={{ alignItems: 'center', justifyContent: 'center' }}>
        <AppButton
          onPress={async () => {
            // TODO Close order
            await closeOrder();
