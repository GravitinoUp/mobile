...
export default function OrderScreen({ navigation, route }: any) {
  const order: IOrder = route.params.order;
      ...
      <AppTopBar style={styles.header}>
        <HStack style={{ alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row' }}>
            <Text style={{ marginEnd: 12 }}>{order?.order_status?.order_status_name}</Text>
            {renderIconSwitch(order?.order_status?.order_status_name)}
          </View>
        </HStack>
        ...
        <AppCard>
          <View style={styles.topRow}>
            <View style={{ flex: 1, flexDirection: 'row', }}>
              <CalendarIcon />
              <Text style={{ marginLeft: 6, fontSize: 14, color: AppColors.bodyDark }}>{`${moment(order?.planned_datetime).format(AppStrings.longDateFormat)}`}</Text>
            </View>
          </View>
          <View style={{ height: 1, width: '100%', borderRadius: 1, borderWidth: 1, borderColor: AppColors.border, borderStyle: 'dashed' }} />
          <View style={styles.cardContent}>
            <View style={{ gap: 4, flexDirection: 'column', paddingBottom: 16 }}>
              <AppTextInput
                value={`${order.order_name ? order.order_name : order.task?.task_name}`}
                readOnly={true}
              ...
              />
              <AppTextInput
                value={`${order.order_description ? order.order_description : order.task?.task_description}`}
                readOnly={true}
                multiline={true}
                minHeight="50px"
            ...
            <AppButton
              ...
              onPress={() => { navigation.navigate("CloseOrderScreen", { order: order }) }} text={"Закрыть задачу"}
              paddingX={40}
              paddingY={10}
            />
          </View>
